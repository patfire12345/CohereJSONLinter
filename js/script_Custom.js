...
const validationButton = document.getElementById("custom-validate");
delimiter.addEventListener("input", () => {
  validationButton.disabled = delimiter.value.length !== 1;
});
...
function lintCustom() {
  ...
  if (lines.length === 0) {
    outputDiv.innerHTML = `<div class="alert alert-danger">Error: No data provided. Please enter some data for validation.</div>`;
    return;
  }
  ...
  if (errorMessages.length > 0) {
    ...
    outputDiv.innerHTML = `<div class="alert alert-success" role="alert">Valid table!</div><table class="table table-bordered"><thead><tr>${headerRow.join(
      "",
